...
```{r}
##load data
client_train<-read.csv("data/train/client_train.csv")
...
```{r}
invoice_train<-read.csv("data/train/invoice_train.csv")
...
```{r}
client_test<-read.csv("data/test/invoice_test.csv")
glimpse(client_test)
```
...
```{r}
invoice_test<-read.csv("data/test/invoice_test.csv")
...
aggregate_by_client_id <- function(invoice_data) {
  ...
  agg_trans <- invoice_data %>%
    group_by(client_id) %>%
    summarise(
      transactions_count = n(), #count num of transactions per client_id
...
train <- train %>%
  select(-c("client_id", "creation_date"))
